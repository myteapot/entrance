...
5. 冷启动检测:
   - 项目有 notes/oracles/ 目录? → 有: 继续 step 6
   - 没有 notes/oracles/ → Phase 0 (新项目初始化, 见下方)
6. diff 检查 notes/oracles/input.md → 有新内容 → 优先处理
...
```
0. 项目没有 notes/oracles/ 目录 → Phase 0 (新项目初始化)
1. notes/oracles/input.md 有新内容 (Human 反馈/新素材) → 优先处理
2. notes/oracles/dialog.md 不完整 → Phase 1
3. notes/specs/ 不齐全 → Phase 2
...
**触发条件**: 项目目录没有 `notes/oracles/`

```
1. 问 Human: 这个项目叫什么? 做什么? (1-2 句话就够)
2. 创建项目骨架:
   - notes/oracles/input.md (Human 的原始素材)
   - notes/oracles/dialog.md (空)
   - notes/oracles/oracle.md (空, 访谈后填写)
...
1. 需求澄清 → `notes/oracles/dialog.md`
2. Spec 编写 → `notes/specs/`
