...
prepare_release_stage() {
  local stage_dir="$1"
  local output_dir="$2"
  local app_version electron_version homepage
  ...
  cp -a "$repo_root/electron/." "$stage_dir/electron/"
  cp -a "$repo_root/src-tauri/icons/." "$stage_dir/icons/"
  cp -a "$repo_root/src-tauri/target/release/entrance" "$stage_dir/entrance"
