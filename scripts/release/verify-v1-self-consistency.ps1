...
if (-not [string]::IsNullOrWhiteSpace($entranceBin)) {
    ...
    $candidateExe = Join-Path $repoRoot "src-tauri\target\debug\entrance.exe"
    $candidatePosix = Join-Path $repoRoot "src-tauri/target/debug/entrance"
...
function Invoke-Entrance {
    ...
    if ($useCargo) {
        & cargo run --manifest-path (Join-Path $repoRoot "src-tauri/Cargo.toml") -- @CommandArgs
    } else {
...
& cargo test --manifest-path (Join-Path $repoRoot "src-tauri/Cargo.toml") --lib
