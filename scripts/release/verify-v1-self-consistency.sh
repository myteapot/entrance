...
manifest_path="$repo_root/scripts/release/reconciliation-batch-01.json"
...
usage() {
  cat <<'USAGE'
Usage:
  scripts/release/verify-v1-self-consistency.sh [--manifest <path>] [--output-dir <path>] [--skip-e2e]
...
if [[ -n "$entrance_bin" ]]; then
  use_cargo=0
elif [[ -x "$repo_root/src-tauri/target/debug/entrance" ]]; then
  entrance_bin="$repo_root/src-tauri/target/debug/entrance"
  use_cargo=0
elif [[ -x "$repo_root/src-tauri/target/debug/entrance.exe" ]]; then
  entrance_bin="$repo_root/src-tauri/target/debug/entrance.exe"
  use_cargo=0
fi
