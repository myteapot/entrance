...
- 本轮继续把 issue activity timeline 推进成 issue-first 控制面：`hive issue timeline <id>` / `hive_issue_timeline` / `entrance://issues/{issue_id}/timeline` / Panel Activity Timeline 现在会暴露 round groups、Blocked/Needs Review human decision surface、primary action、retry/review/cancel/comment command、confirmation policy 和 issue-control/review-queue resource。
...
- Productize issue timeline：筛选/折叠、远端 issue comment 映射、timeline item permalink、inline decision 的操作回执/刷新状态，以及更强的 blocked action provenance。
...
优先把 issue timeline 的 inline decision 和 Evidence Drilldown/Manifest 接到真实 agent/connector 产物：操作后刷新、真实 artifact manifest 生成与内容校验、完整 transcript 展开、blocker decision workflow。
