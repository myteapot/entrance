...
use entrance_core::dispatch_host::{CreateTaskRequest, ForgeTaskMetadata};
use serde::{Deserialize, Serialize};
use serde_json::{json, Value};

use crate::{
    core::action::ActorRole,
    hosts::plugins::forge::{allocate_agent_slot_worktree, ForgePlugin},
};

const BOOTSTRAP_DEV_RUNTIME_MODE: &str = "bootstrap_dev_runtime_task";

struct SpawnedMcpStdioClient {
    child: Child,
...
impl SpawnedMcpStdioClient {
    fn spawn(app_data_dir: &Path, actor_role: ActorRole) -> Result<Self> {
        let mut command = Command::new(resolve_mcp_executable()?);
        command.arg("stdio");
    ...
    fn call_tool(&mut self, name: &str, arguments: Value) -> Result<Value> {
            ...
            .unwrap_or(false)
        {
            ...
            anyhow::bail!("child MCP tool `{name}` failed: {message}");
        }
...
fn resolve_cli_executable() -> Result<std::path::PathBuf> {
    if let Some(path) = std::env::var_os("ENTRANCE_CLI_BIN") {
        return Ok(path.into());
    }
...
fn build_bootstrap_dev_runtime_request(
    ...
    let command = resolve_cli_executable()?.to_string_lossy().to_string();
