...
use anyhow::Result;
use entrance_core::{
    data_store::{DataStore, StoredRuntimeHost, UpsertOwnedWorktree, UpsertRuntimeHost},
    environment_runtime::{
        list_owned_worktrees, OwnedWorktreeRegistryReport, RuntimeEnvironmentReport,
    },
};
use walkdir::WalkDir;
...
mod tests {
    ...
    use crate::{paths::HarnessPaths, plugins::forge};

    use super::{normalize_path, record_runtime_environment};
    ...
    fn runtime_environment_tracks_host_and_worktree_registry() -> Result<()> {
        ...
        let paths = HarnessPaths::new(&temp.app_data_dir);
        paths.ensure_layout()?;
        let migration_plan = MigrationPlan::new(forge::migrations());
        let store = DataStore::open(paths.db_path(), migration_plan)?;
