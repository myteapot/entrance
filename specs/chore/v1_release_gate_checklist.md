...
```bash
./scripts/release/verify-v1-self-consistency.sh --skip-e2e
```

```powershell
./scripts/release/verify-v1-self-consistency.ps1 -SkipE2E
```

- Use a prebuilt Entrance binary instead of `cargo run`:

```bash
ENTRANCE_EXE_PATH=/path/to/entrance ./scripts/release/verify-v1-self-consistency.sh
```
