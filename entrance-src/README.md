...
above Evidence Drilldown, Evidence Manifest, Activity Timeline, Runtime
Preflight, and Worker Lifecycle. After state-changing Panel actions, the board
refresh also forces a fresh selected-issue read of Transition Policy, Loop
Dashboard, Evidence Drilldown, Evidence Manifest, Activity Timeline, Runtime
Preflight, and Worker Lifecycle.
MCP also exposes `entrance_loop_control` and
`entrance://loops/{loop_id}/control`, a Reviewer-ready control packet that
...
typed confirmation receipts. Compact issue trace/operator summaries expose the
same admission proof, including action, gate, from/to status, policy registry
resource, transition-policy resource, and confirmation requirement.
...
permalinks, a Blocked/Needs Review human decision surface, operator confirmation
receipt provenance, and next actions; MCP exposes it as
`entrance://issues/{issue_id}/timeline`, and the Panel selected issue renders it
below Evidence Manifest. `hive issue timeline-item <id> <item-id>` and
`entrance://issues/{issue_id}/timeline/items/{item_id}` read one permalinked
...
grouping, operator confirmation receipts, and the current issue-level human
decision contract when the issue is `Blocked` or `Needs Review`.
`hive issue timeline-item <id> <item-id>` returns the single permalinked
timeline item report for agents or operators who need to cite one comment,
evidence row, verdict, or operator decision without rereading the whole feed.
...
action coverage. The same transition admission action/gate/resource proof is
also projected into issue trace operator events for Panel and MCP control
packets. The
...
rejects drifted `target_binding` admission receipts; it also recomputes missing
receipts, gate pass/fail, gate reason, and admitted/rejected result bindings
from the current packet/policy ledger.
