...
async fn handle_invoke(
    ...
    match command.as_str() {
        ...
        "hive_loop_evidence_manifest" => {
            let id = args
                .get("id")
                .and_then(|value| value.as_i64())
                .context("hive_loop_evidence_manifest requires `id`")?;
            Ok(serde_json::to_value(
                state.services.hive.loop_evidence_manifest(id)?,
            )?)
        }
        "hive_loop_audit" => {
            let id = args
        ...
        "hive_loop_worker_lifecycle" => {
            let id = args
                .get("id")
                .and_then(|value| value.as_i64())
                .context("hive_loop_worker_lifecycle requires `id`")?;
            Ok(serde_json::to_value(
        ...
        "hive_issue_timeline_item" => {
            let issue_id = args
                .get("issueId")
                .or_else(|| args.get("issue_id"))
                .or_else(|| args.get("id"))
                .and_then(|value| value.as_i64())
                .context("hive_issue_timeline_item requires `issueId`")?;
            let item_id = args
                .get("itemId")
                .or_else(|| args.get("item_id"))
                .and_then(|value| value.as_str())
                .context("hive_issue_timeline_item requires `itemId`")?;
            Ok(serde_json::to_value(
