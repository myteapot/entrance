...
| GUI | GUI shows Runtime, Drawer, Hive, Launcher, and a minimal Linear-like Panel for loop issues. Issue cards show round-aware packet/admission/evidence/verdict trace chips, and the connector queue can plan then execute local mirror publishes with an execution receipt visible in comments/evidence. | Full loop dashboard for observing rounds, roles, status, evidence drilldown, verdicts, and human review points. |
