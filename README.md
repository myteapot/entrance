...
当前仓库已经具备一个可运行的本地闭环原型：可以用 `hive loop start --runtime codex --compact` 一条命令创建并运行 issue 绑定的 loop，也可以拆成 `loop create` 和 `issue run` 两步；运行时会串行跑 `Explorer -> Doer -> Evaluator`，把 packet、admission、worker receipt、evidence、verdict 和 issue comment 写入 SQLite，并在本地 Panel/CLI 中查看状态。
