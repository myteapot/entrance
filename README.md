...
- 更完整的 evidence drilldown 产品化：当前已有 `evidence_drilldown.v1` 和 Panel 聚焦视图，能展示 worker receipt、transcript/payload excerpt、remote receipt 摘要、artifact/path hint、payload key diff、blocker 和 human decision actions；还缺可展开的完整 transcript、真实远端 receipt 归档、artifact manifest 收集/校验、payload schema diff 和 blocker decision workflow。
