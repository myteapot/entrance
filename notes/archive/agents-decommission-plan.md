...
- `notes/oracles/oracle.md`
- `notes/oracles/dialog.md`
- `notes/specs/backend.md`
- `notes/specs/milestones.md`

These should continue to exist as source-controlled design truth.

### 2.2 Imported copy-first, source still preserved in `.agents`

The following hot files now have repo-side canonical copies under `Entrance/notes/harness/bootstrap/`, while the original `.agents` files remain preserved:
...
The following recovered documents now also have repo-side copies under `Entrance/notes/specs/recovery/`, while the original `.agents` files remain preserved:
...
- `hosts/desktop/tauri/src/hosts/plugins/forge/mod.rs`
  - prompt generation now reads Entrance-owned `notes/harness/bootstrap/duet/SKILL.md`
  - managed worktree discovery now uses `~/.entrance/worktrees/{project}/feat-{ISSUE}` as its only runtime owner path
- `surfaces/gui/renderer/pages/Forge.tsx`
  - UI now surfaces `Entrance-owned notes/harness/bootstrap prompt`
...
- `notes/oracles/` for product truth and decision extraction
- `notes/specs/` for implementation and migration design

Recovered recovery-era docs should be copied into a source-controlled recovery area, recommended as:

- `notes/specs/recovery/`
...
- copy hot role and rule files from `.agents` into `Entrance/notes/harness/bootstrap/`
...
- UI now advertises `Entrance-owned notes/harness/bootstrap prompt` as the active source
...
```text
rg -n "\.agents|control\.py|db\.py" src hosts/desktop/tauri
```
...
- prompt generation now follows the Entrance-owned `notes/harness/bootstrap/` path
