...
5. **Arch 独占 `notes/specs/` 写权**。Human 独占 `notes/oracles/input.md` (素材输入)。Arch 独占 `notes/oracles/dialog.md` (记录) + `notes/oracles/oracle.md` (提炼真相, Human 审批)。Agent 只读 + 更新 Linear。
6. **Arch 每次启动时 diff 检查 `notes/oracles/input.md`**。
...
1. 读 `notes/oracles/input.md` (原始素材) + diff 检查是否有新内容
2. 多轮结构化访谈 (每轮 5-8 问)
3. Human 原文逐字记录到 `notes/oracles/dialog.md`
4. 从访谈中提炼真相 → 写入 `notes/oracles/oracle.md` → Human 确认 → Phase 2

## Phase 2: 规格设计 (Spec)

**执行者**: Arch | **Human 参与**: 审阅

按序产出 `notes/specs/` 6 份文档 → Human 审阅 → 待定项清零
