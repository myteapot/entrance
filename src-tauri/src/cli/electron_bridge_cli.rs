...
impl ElectronBridgeRuntime {
    ...
    async fn handle_invoke(&self, command: &str, args: Value) -> Result<Value, String> {
        match command {
            ...
            "dashboard_summary" => to_json_value(commands::build_dashboard_summary(
                &self.dashboard_ui,
                &self.data_store,
            )?),
            ...
            "nota_approve_prayer" => {
                ...
                to_json_value(commands::nota_prayer::approve_prayer(
                    &self.data_store,
                    args.allocation_id,
                )?)
            }
            "nota_reject_prayer" => {
                ...
                to_json_value(commands::nota_prayer::reject_prayer(
                    &self.data_store,
                    args.allocation_id,
                    args.reason,
                )?)
